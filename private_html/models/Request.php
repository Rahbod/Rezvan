...
//            'bathroom' => ['INTEGER', ''],
    ...
    public function getPdf($to_file = false)
    {
        $content = app()->controller->renderPartial('//request/pdf', ['model' => $this, 'pdf_mode' => true]);
        ...
        if ($to_file) {
            ...
            $pdf->filename = alias('@webroot/uploads/pdf/Request file - ' . $this->id . '.pdf');
            $pdf->render();
            return $pdf->filename;
        } else {
            $pdf->filename = 'Request file - ' . $this->id . '.pdf';
            return $pdf->render();
        }
    }
    ...
    public function afterDelete()
    {
        ...
        $pdfFile = alias('@webroot/uploads/pdf/Request file - ' . $this->id . '.pdf');
        if (is_file($pdfFile))
            @unlink($pdfFile);
    }
}